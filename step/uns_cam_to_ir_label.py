...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(tqdm(infer_data_loader, position=process_id, desc=f'[PID{process_id}]')):
        ...
        cam_dict = np.load(os.path.join(args.uns_cam_out_dir, img_name + '.npy'), allow_pickle=True).item()
        ...
        imageio.imwrite(os.path.join(args.uns_ir_label_out_dir, img_name + '.png'),

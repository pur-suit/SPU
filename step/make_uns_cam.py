...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(tqdm(data_loader, position=process_id, desc=f'[PID{process_id}]')):
            ...
            label_uns = Image.open(os.path.join(args.ir_label_out_dir, img_name + '.png'))

            strided_size = imutils.get_strided_size(size, 4)

            label_uns_down = np.array(label_uns.resize((strided_size[1], strided_size[0]), resample=Image.NEAREST))
            label_uns_up = np.array(label_uns.resize((size[1], size[0]), resample=Image.NEAREST))
            ...
            strided_mask_bg = torch.tensor(label_uns_down == 0).unsqueeze(0).expand(strided_cam.shape).numpy()
            highres_mask_bg = torch.tensor(label_uns_up == 0).unsqueeze(0).expand(highres_cam.shape).numpy()

            strided_cam[~strided_mask_bg] = strided_cam_norm[~strided_mask_bg]
            highres_cam[~highres_mask_bg] = highres_cam_norm[~highres_mask_bg]

            # save cams
            np.save(os.path.join(args.uns_cam_out_dir, img_name + '.npy'),
                    {"keys": valid_cat, "cam": torch.tensor(strided_cam), "high_res": highres_cam})


def run(args):

    model = getattr(importlib.import_module(args.uns_network), 'CAM')()
    model.load_state_dict(torch.load(args.uns_weights_name + '.pth'), strict=True)
    model.eval()
